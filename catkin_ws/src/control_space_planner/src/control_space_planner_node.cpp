...
MotionPlanner::MotionPlanner(ros::NodeHandle& nh) : nh_(nh)
{
  ...
  subEgoOdom = nh.subscribe("/orb_slam3/odom",1, &MotionPlanner::CallbackEgoOdom, this);
  ...
  pubGoalReached_ = nh_.advertise<std_msgs::Bool>("/planner/goal_reached", 1, false);
...
void MotionPlanner::CallbackGoalPoint(const geometry_msgs::PoseStamped& msg)
{
  ...
  this->bGetGoal = true;
  goal_reached_sent_ = false; 
  ROS_INFO_STREAM_THROTTLE(1.0, "============> [GoalPoint] (x,y,yaw)=("
  << msg.pose.position.x << "," << msg.pose.position.y << "," << goal_yaw
  << ")");
}
...
void MotionPlanner::PublishCommand(std::vector<Node> motionMinCost)
{
  ...
  // --- [NEW EDGE CASE] Detect close front obstacle + large delta ---
  bool use_reactive_turn = false;
  double selected_delta = motionMinCost.back().delta;

  if (bGetLocalNode && std::abs(selected_delta) > 20.0 * M_PI / 180.0) { // >20 deg
      // Check if there's an obstacle very close in front (e.g., within 0.2 m)
      double dist_to_obstacle = std::sqrt(localNode.x * localNode.x + localNode.y * localNode.y);
      // But note: localNode is goal, not obstacle!
      // Instead, approximate: if ANY primitive collided early, and we picked a turning one
      // → use heuristic: if motion is short and colliding
      if (motionMinCost.size() <= 2 && motionMinCost.back().collision) {
          use_reactive_turn = true;
      }
  }

  if (use_reactive_turn) {
      // Aggressive mecanum response: strafe + rotate
      double base_speed = 0.25; // tune as needed
      command.linear.x = 0.0; // no forward
      command.linear.y = base_speed * sin(selected_delta); // lateral
      command.angular.z = 1.2 * selected_delta; // direct angular from delta
      ROS_WARN_THROTTLE(0.5, "[ReactiveTurn] delta=%.1f deg", selected_delta * 180 / M_PI);
  } else {
      // --- [ORIGINAL LOGIC] ---
  ...
  if (bGetGoal && bGetLocalNode) {
    ...
    if (distToGoal < this->ARRIVAL_THRES) {
      ...
      ROS_INFO("=========================================== ARRIVED ===========================================");
      if (!goal_reached_sent_) {
        std_msgs::Bool m; m.data = true;
        pubGoalReached_.publish(m);
        goal_reached_sent_ = true;
        ROS_INFO("[PLANNER] Goal reached → notifying GraphPlanner");
      }
    }
  }
...
std::vector<std::vector<Node>> MotionPlanner::GenerateMotionPrimitives(nav_msgs::OccupancyGrid localMap)
{
  ...
  */
  ...
  for (int i = 0; i < num_candidates; i++) {
    ...
    const double cost_control = std::abs(angle_delta);
    // <<MODIFICATION>> init start node
    Node startNode(0, 0, 0, 0, angle_delta, cost_control, 0, 0, -1, false);
                                              ...
                                              nav_msgs::OccupancyGrid localMap)
{
  ...
  */
  ...
  while (progress < maxProgress) {
    ...
    // <<MODIFICATION>>
    double vx = this->MOTION_VEL * cos(startNode.delta);
    double vy = this->MOTION_VEL * sin(startNode.delta);
    currMotionNode.x += (vx * cos(currMotionNode.yaw) - vy * sin(currMotionNode.yaw)) * this->TIME_RESOL;
    currMotionNode.y += (vx * sin(currMotionNode.yaw) + vy * cos(currMotionNode.yaw)) * this->TIME_RESOL;
    ...
    if (CheckCollision(collisionPointNodeMap, localMap)) {
      ...
      ROS_WARN_STREAM_THROTTLE(1.0, "[Rollout FARAWAY] COLLISION at (grid) "
    ...
    Node collisionPointNode_2(currMotionNode.x, currMotionNode.y, currMotionNode.z, currMotionNode.yaw, 
                           currMotionNode.delta, 0, 0, 0, -1, false);
    Node collisionPointNodeMap_2 = LocalToPlannerCorrdinate(collisionPointNode_2);

    if (CheckCollision(collisionPointNodeMap_2, localMap)) {
      // - do some process when collision occurs.
      // - you can save collision information & calculate collision cost here.
      // - you can break and return current motion primitive or keep generate rollout.
      // <<MODIFICATION>>
      ...
      motionPrimitive.push_back(currMotionNode);
      progress += this->DIST_RESOL;
      ROS_WARN_STREAM_THROTTLE(1.0, "[Rollout TOO CLOSE] COLLISION at (grid) "
      << collisionPointNodeMap_2.x << "," << collisionPointNodeMap_2.y
      << "  prim_len=" << motionPrimitive.size());

      return motionPrimitive;
    }

    double safeaheadX = currMotionNode.x + this->SAFE_LOOKAHEAD_DIST * cos(currMotionNode.yaw);
    double safeaheadY = currMotionNode.y + this->SAFE_LOOKAHEAD_DIST * sin(currMotionNode.yaw);
    Node collisionPointNode_3(safeaheadX, safeaheadY, currMotionNode.z, currMotionNode.yaw, 
                           currMotionNode.delta, 0, 0, 0, -1, false);
    Node collisionPointNodeMap_3 = LocalToPlannerCorrdinate(collisionPointNode_3);

    if (CheckCollision(collisionPointNodeMap_3, localMap)) {
      // - do some process when collision occurs.
      // - you can save collision information & calculate collision cost here.
      // - you can break and return current motion primitive or keep generate rollout.
      // <<MODIFICATION>>
      ...
      motionPrimitive.push_back(currMotionNode);
      progress += this->DIST_RESOL;
      ROS_WARN_STREAM_THROTTLE(1.0, "[Rollout NEARBY] COLLISION at (grid) "
      << collisionPointNodeMap_3.x << "," << collisionPointNodeMap_3.y
      << "  prim_len=" << motionPrimitive.size());
...
std::vector<Node> MotionPlanner::SelectMotion(std::vector<std::vector<Node>> motionPrimitives)
{
  ...
  */
  const double W_GOAL = 1.5;
  const double W_HEAD = 0.2;
  const double W_STEER = 0.003;
  const double W_TRAV = this->W_COST_TRAVERSABILITY; 
  const double W_PROG = 0.5;
  double W_EARLY_TURN = 10.0;
  ...
  if (motionPrimitives.size() != 0) {
    ...
    for (auto& motionPrimitive : motionPrimitives) {
      ...
      // <<MODIFICATION>>
      if (motionPrimitive.empty()) continue;
      // if (motionPrimitive.back().collision) continue;
      ...
      double J_steer;
      if (motionPrimitive.back().collision) {
          // Prefer LARGE steering when colliding → NEGATIVE cost for |delta|
          J_steer = (-1)*std::abs(motionPrimitive.back().delta); // ← note the minus!
      } else {
          // Normal: penalize steering
          J_steer = std::abs(motionPrimitive.back().delta);
      }
      double J_early_turn = std::abs(motionPrimitive[0].delta);
      ...
      if (motionPrimitive.back().collision) {
          // Reduce cost for primitives that turn/strafe immediately
          cost_total -= W_EARLY_TURN * J_early_turn; // note: subtraction!
      }
      ...
      << " J_trav=" << J_trav << " J_early_turn=" << J_early_turn << " prog=" << progress_m
      << " => total=" << cost_total
